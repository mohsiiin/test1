...
def aligarh(request):
    ...
    return render(request,'ourcity.html',{'city':city,'rests':rests})

@login_required(login_url='/welcome/error')
def delhi(request):
    city2 = City.objects.get(id=2)
    rests2 = RestaurantsDelhi.objects.all()
    return render(request,'ourcity.html',{'city':city2,'rests':rests2})

@login_required(login_url='/welcome/error')
def agra(request):
    city3 = City.objects.get(id=3)
    rests3 = RestaurantsAgra.objects.all()
    return render(request,'ourcity.html',{'city':city3,'rests':rests3})
